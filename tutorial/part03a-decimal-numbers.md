...
haven't talked much about the polyglot aspect of Truffle and we're not
going to go into it now. 

## Premature optimization for fun and education

Now, by using doubles everywhere we've slowed our calculator down!
It's obviously a lot slower to add 2 and 2 together when they are
represented using doubles instead of longs! Ok. So it really doesn't
matter at all for our trivial case, but I let's pretend it matters
since it's such a good way to introduce the concept of specialization.
...
Great! Now we just need to implement the remaining nodes. I'll leave
that as an exercise, and if you get stuck you can always cheat and
look at the final source code for this chapter.
